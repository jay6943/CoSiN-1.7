...
def mask_1(fp):
  ...
  _, y1 = y2x2.chips(xo, y1 + cfg.sch * 1.5, dev.arange(49, 53, 0.5))
  ...
  _, y1 = tap.chips(xo, y1 + cfg.sch, dev.arange(1.16, 1.46, 0.1))
...
def mask_2(fp):
  ...
  _, y1 = tip.chip(xo, yo + cfg.sch, cfg.size, 0.36)
  _, y1 = dci.chip(xo, y1 + cfg.sch * 1.5, cfg.size)
  ...
  _, y1 = qdc.chip(xo, y1 + cfg.ch, cfg.size)
  _, y1 = y2x2.chip(xo, y1 + cfg.sch * 0.5 + cfg.ch, cfg.size)
  _, y1 = tip.chip(xo, yo + cfg.size - cfg.sch, cfg.size, 0.36)
